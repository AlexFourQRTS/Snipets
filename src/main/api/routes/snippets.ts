  ...
  .patch(
    ...
    ({ params, body }) => {
      ...
      if (updateFields.length === 0) {
        return new Response(
          JSON.stringify({ message: 'Need at least one field to update' }),
          {
            status: 400,
  ...
  .patch(
    ...
    ({ params, body }) => {
      ...
      if (!contentsResult.changes) {
        return new Response(
          JSON.stringify({ message: 'Snippet content not found' }),
          {
            status: 404,
            headers: { 'Content-Type': 'application/json' },
          },
    ...
    () => {
      try {
        ...
        if (deletedSnippets.length === 0) {
          return new Response(
            JSON.stringify({ message: 'No snippets in trash' }),
            {
              status: 404,
              headers: { 'Content-Type': 'application/json' },
            },
        ...
        return new Response(
          JSON.stringify({ error: 'Failed to delete trash' }),
          {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
          },
