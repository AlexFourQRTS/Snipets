...
const snippetsCache = new Map<string, SnippetsResponse>()
...
async function getSnippets(query?: SnippetsQuery) {
  const queryToUse = query || queryByLibraryOrFolderOrSearch.value
  const cacheKey = JSON.stringify(queryToUse)
  
  // Проверяем кэш
  const cached = snippetsCache.get(cacheKey)
  if (cached) {
    if (isSearch.value) {
      snippetsBySearch.value = cached
    } else {
  ...
  const { data } = await api.snippets.getSnippets(queryToUse)
  
  // Сохраняем в кэш
  snippetsCache.set(cacheKey, data)
  
  // Очищаем старые записи из кэша
  setTimeout(() => {
    snippetsCache.delete(cacheKey)
  }, CACHE_TTL)
